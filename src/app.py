'''
1. モジュールのインポート
    animal.py から 前処理とネットワークの定義を読み込み
2. 推論する
3. 推論したラベルから犬か猫かを返す関数
4. Flask のインスタンスを作成
5. 拡張子が適切かどうかをチェック
6. URL にアクセスがあった場合の挙動の設定

'''

# 必要なモジュールのインポート
import torch
from animal import transform, Net # animal.py から前処理とネットワークの定義を読み込み-> predict 関数で使用する
...
def predicts():
    # リクエストがポストかどうかの判別
    if request.method == 'POST': # 判定ボタンが押された場合
        # ファイルがなかった場合の処理
        if 'filename' not in request.files:
            return redirect(request.url) # リクエストしてきたURLにリダイレクト
        # データの取り出し
        file = request.files['filename']
        # ファイルのチェック
        if file and allwed_file(file.filename): # 当てはまる場合
            ...
            # 入力された画像に対して推論
            pred = predict(image)
            animalName_ = getName(pred) # 推論したラベルから犬か猫かを返す関数を使用
